...
class UnifiCamBase(metaclass=ABCMeta):
    ...
    async def trigger_motion_start(
        ...
        # Allow Smart Detect events even if motion is already active (to upgrade generic motion to Smart Detect)
        # But only allow one motion event at a time for non-Smart Detect events
        ...
        if not self._motion_event_ts or is_upgrade:
            ...
            try:
                snapshot_source = await self.get_snapshot()
                if snapshot_source and snapshot_source.exists():
                    shutil.copyfile(snapshot_source, motion_snapshot_path)
                    self._motion_snapshot = Path(motion_snapshot_path)
                else:
                    self.logger.warning(f"⚠️  Snapshot source not available: {snapshot_source}")
            ...
            if object_type:
                ...
                # So we include motion data (levels, motionHeatmap, motionSnapshot) even for Smart Detect events
                
                # Get snapshot dimensions if available
                snapshot_width = 640
                snapshot_height = 360
                if self._motion_snapshot and self._motion_snapshot.exists():
                    width, height = self._get_image_dimensions(self._motion_snapshot)
    ...
    async def trigger_motion_stop(self) -> None:
        ...
        if motion_start_ts:
            ...
            if motion_object_type:
                # Get actual snapshot dimensions if available
                snapshot_width = 640
                snapshot_height = 360
                if self._motion_snapshot and self._motion_snapshot.exists():
                    width, height = self._get_image_dimensions(self._motion_snapshot)
    ...
    def _get_image_dimensions(self, image_path: Path) -> tuple[int, int]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to get image dimensions from {image_path}: {e}")
            return (640, 360)  # Default fallback dimensions
    ...
    async def process_hello(self, msg: AVClientRequest) -> None:
        ...
        self.logger.info(
            f"Controller version: {controller_version}, camera model: {self.args.model}"
        )
            ...
            msg["messageId"],
            {
                ...
                "video": {
                    ...
                    "video1": {
                        ...
                        "bitRateCbrAvg": 10000000,
                        "bitRateVbrMax": 12000000,
                        "bitRateVbrMin": 4000000,
                        ...
                        "isCbr": False,
                        "maxFps": 30,
                        "minClientAdaptiveBitRate": 4000000,
                        "minMotionAdaptiveBitRate": 4000000,
                        "nMultiplier": 6,
                        "name": "video1",
                        "sourceId": 0,
                        "streamId": 1,
                    ...
                    "video2": {
                        ...
                        "bitRateCbrAvg": 4000000,
                        "bitRateVbrMax": 5000000,
                        "bitRateVbrMin": 2000000,
                        "currentVbrBitrate": 4000000,
                        ...
                        "isCbr": False,
                        "maxFps": 30,
                        "minClientAdaptiveBitRate": 2000000,
    ...
    async def process_snapshot_request(
        ...
        if path and path.exists():
            ...
            if snapshot_size == 0:
                self.logger.warning(f"⚠️  Snapshot file {path} exists but is empty (0 bytes)")
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    ...
                    self.logger.info(f"✅ Uploaded {snapshot_type} from {path} (size: {snapshot_size} bytes)")
    ...
    async def process(self, msg: bytes) -> bool:
        ...
        if fn == "ubnt_avclient_time":
            ...
            res = self.gen_response(
                "AudioAgentChangeTuning", response_to=m["messageId"]
            )
